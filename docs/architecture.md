...
SD-DOS全体の構成と、ストリーム読み出しAPI追加(検討中)の位置づけを俯瞰する。実装の進行に合わせて更新する前提の文書である。
...
任意ファイルを少しずつ読み出す汎用ストリームアクセス機能をSD-DOSへ追加する予定である。利用例はVGM再生サンプル(別途作成)である。

呼び出し口は機械語APIとし、プログラム先頭の固定位置に置くジャンプテーブルで公開する(詳細は [ストリーム読み出しの呼び出し口](design/entry-point.md) を参照)。エントリ構成案はオープン、1バイト取得、必要に応じて状態取得・クローズである。
...
複数クラスタにまたがるファイルで2クラスタ目以降の読み込みが正しくない。ストリーム読み出しAPIの実装に先立ってバグ修正として対応する。詳細は [複数クラスタ読みの既存挙動の確認結果](design/multicluster-read.md) を参照する。
