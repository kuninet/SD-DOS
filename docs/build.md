...
* `6000H`〜`604BH`: ローダ。60バイトのルーチンを高位RAM(`D000H`)へコピーして実行し、ポート`E2H`のバンク切り替えを使ってBASIC ROM(`0000H`〜`5FFFH`)を裏RAMへコピーし、続いて`604CH`にあるSD-DOS本体(長さ`BODY_LEN`。オリジナルは`18F0H`)を`6000H`へコピーしてから`6000H`へジャンプする
...
* `docs/`: ビルド手順(この文書)
* `docs/design/`: 調査と設計の文書(SD-DOS本体)
* `docs/vgm/`: VGM再生サンプルの設計文書
* `scripts/`: ビルド補助スクリプト
