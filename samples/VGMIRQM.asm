...
;-------------------------------------------------
;再生ループ(マーカー入り版: 進行を画面上で可視化する)
;・各分岐の頭で1文字出力。MAXCNT 回 LOOP を回したら ABORT
;・マーカー一覧:
;    P : PLAY 突入
;    . : .LOOP の頭(コマンド取得直前)
;    O : .OPN 突入
...
;    [ : WAIT_DE_POLL 突入
;    ] : WAIT_DE_POLL 戻り
;    X : LOOP_COUNT 上限到達で ABORT
;-------------------------------------------------
PLAY:
	LD	A,'P'
	RST	18H
	;ループカウンタを初期化(暴走/無限ループ対策)
	LD	HL,0
	LD	(LOOP_CNT),HL
.LOOP:
	;LOOP上限到達チェック
...
MSG_HDR:	DB	CR,LF,"-- VGM (MARK) --",CR,LF,00H
MSG_COLON:	DB	": ",00H
...
MSG_LOOPFULL:	DB	CR,LF,"LOOP MAX",CR,LF,00H

;-------------------------------------------------
SAVSP:		DS	2
BLKSZ:		DS	4
LOOP_CNT:	DS	2			;PLAY.LOOPの回転数(マーカー版の暴走検知)
RBUF:		DS	RBUF_SIZE
RBUF_END	EQU	$
RB_RDP:		DS	2
RB_WRP:		DS	2
